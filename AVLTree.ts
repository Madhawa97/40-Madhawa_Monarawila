...
export class AVLTree {
  ...
  public getTreeHeight(node: AVLTree ): number {
  ...
  public search(node: AVLNode | null, searchKey: number): boolean{
    let valueFound = false;
    
    const searchIn = (node: AVLNode| null, searchKey: number): AVLNode =>{
        if (node){
  ...
  private removeData(node: AVLNode | null, key: number): AVLNode {
    if (!node) {
